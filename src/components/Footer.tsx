...
import { FaPhone, FaRegAddressCard } from 'react-icons/fa6';
import { MdOutgoingMail,MdHomeWork } from 'react-icons/md';
...
const Footer: React.FC = () => {
    const footerNavs: FooterNavItem[] = [
        {
            label: 'About Us',
            items: [{ href: 'javascript:void()', name: 'About' }]
        },
        {
            label: 'Gallery',
            items: [{ href: 'javascript:void()', name: 'Blog' }]
        },
        {
            label: 'Mission',
            items: [{ href: 'javascript:void()', name: 'Team' }]
        },
       
        {
            label: 'Contact-Us',
            items:  [{href: 'javascript:void()', name: 'Support' }]
        }
    ];
            ...
            <div className="gap-6 justify-between md:flex">
                <div className="flex-1">
                    <div className="max-w-xs">
                        <Image src="/logo.png" className="w-32" width={700} height={700} alt='kux v' />
                        <p className="leading-relaxed mt-2 text-[15px]">
                         Aalam expert technicians are your trusted solution for all your electric motor repair needs.
                        </p>
                    </div>
                    <form onSubmit={(e) => e.preventDefault()}>
                        ...
                        <div className="max-w-sm flex items-center rounded-md gap-2">
                            <input 
                                type="email"
                                placeholder="Enter your email"
                                className="w-full p-2.5 outline-none rounded-md border-2 dark:border-none"
                            />
                            <button
                                className="p-2.5 rounded-md bg-indigo-600 outline-none shadow-md focus:shadow-none sm:px-5"
                            >
                                Subscribe
                            </button>
                        </div>
                ...
                <div className="flex flex-col gap-6 mt-6 text-gray-600  dark:text-white">
                    <div className='flex justify-between md:flex-row sm:flex-col sm:gap-4 gap-8 text-xl'>                    
                    {footerNavs.map((el, idx) => (
                        <ul key={idx} className='font-bold hover:text-indigo-600 hover:scale-105 transition-all ease-in-out duration-300'>
                            <li>
                                <Link 
                                    href={el.items[0]?.href || '#'}
                                    className="hover:underline"
                                >
                                    {el.items[0]?.name || el.label}
                                </Link>
                            </li>
                        </ul>
                    ))}
  ...
  <h1 className="flex justify-center font-bold p-2 tracking-widest text-2xl underline">CONTACT INFORMATION</h1>
  <div className="flex flex-col items-start gap-4 px-3">
 
    <div className="flex flex-col justify-center gap-4 sm:flex sm:w-full md:flex-row">
      <FaPhone /> 
      <p>+977-phone-number</p>  

    <MdOutgoingMail />
      <p>youremail@example.com</p>  
    </div>
    <div className='mt-6 '>
    <span className='flex justify-center'><h2 className='flex font-bold text-lg gap-3'><MdHomeWork/> Address</h2></span>
    <p className='sm:text-sm'>
      Jeetpur-Bara, Madhesh Pradesh, Nepal. Pin-code: 44400
            ...
            <div className="mt-8 py-6 border-t items-center justify-between sm:flex">
                <div className="md:mt-4 sm:text-center sm:flex sm:justify-center">
